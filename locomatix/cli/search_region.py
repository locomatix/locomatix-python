...
def search_region():
  ...
  for batch in lxclient._search_region_iterator(region, from_feed, allow_error=True):
    if batch.status > httplib.OK:
      dprint(args, batch, "error: failed to retrieve search region list - %s" % (batch.message))
      continue
